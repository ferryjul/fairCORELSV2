...
static std::function<bool(Node*, Node*)> filtering_based_cmp = [](Node* left, Node* right) {
    return left->get_opt_bound() >= right->get_opt_bound(); // <- the good one
};
...
extern void bbound_begin(CacheTree* tree, Queue* q, rule_t* G_incons_min_errs, int upper_bound_filtering, int pruning_memoisation);

extern std::vector<unsigned long> bbound_end(CacheTree* tree, Queue* q, PermutationMap* p, bool early, rule_t* rules, rule_t* labels, int exitCode, std::string fileName, int verbose_pruning);
